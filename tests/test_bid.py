...
def test_bid_partial(accounts, usdc, market, factory, locker, token, bidder, whale, borrower, backer, lender, loan_contract, loanInfo, amount) : 
    factory.proposeLoan(loanInfo , {'from' : borrower})
    # TO DO - Back Loan 
    chain.mine(1)
    token.approve(locker, 10000, {'from' : backer})
    ...
    loan.approve(market, 0, {'from' : lender})
    lenderBal = usdc.balanceOf(lender)
    market.matchBid(0, amount / 2, 0, {'from' : lender})
    assert usdc.balanceOf(lender) == lenderBal + amount / 2
    #assert loan.ownerOf(0) == bidder


def test_remove_bid(accounts, usdc, market, factory, locker, token, bidder, whale, borrower, backer, lender, loan_contract, loanInfo, amount) : 
    factory.proposeLoan(loanInfo , {'from' : borrower})
    # TO DO - Back Loan 
    chain.mine(1)
    token.approve(locker, 10000, {'from' : backer})
    ...
    market.removeOrder(0, {'from' : bidder})

    assert usdc.balanceOf(bidder) == bidderBal
