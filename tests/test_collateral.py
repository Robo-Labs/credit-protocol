...
def test_collateral_liquidation(accounts, usdc, weth, factory, locker, token, borrower, backer, lender, loan_contract, loanInfoCollateral, amount) : 
    weth.approve(factory, amount, {'from' : borrower})
    ...
    loan.liquidateCollateralAndRepay(loan.collateralAmt(), loan.calcTotalDue(), borrower, {'from' : borrower})

    assert loan.loanRepaid()
    assert loan.loanFinal()
    amtOwed = loan.calcTotalDue()
    assert amtOwed == 0
    loan.withdraw(0, {'from' : lender})
